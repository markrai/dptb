...
CHUNK_DAYS = 30
FALLBACK_START_DATE = datetime.now() - timedelta(days=90)
RATE_LIMIT_DELAY = 2
...
def choose_fitbit_period(days: int) -> str:
    ...
    if d <= 30:
        return '30d'
    if d <= 90:
        return '30d'  # Use 30d for 90 days - more reliable than 3m
    if d <= 366:
        return '1y'
    return 'max'

def fetch_resting_hr_period(end_str: str, period: str, token: str):
    url = f"https://api.fitbit.com/1/user/-/activities/heart/date/{end_str}/{period}.json"
    ...
    for attempt in range(3):
        ...
        if res.status_code == 200:
            ...
            if reset_s > 0:
                ...
                wait_until_next_hour_with_countdown("Rate limited by Fitbit (RHR)")
            continue
            ...
            print(f"WARNING: No RHR data available for period {period} ending {end_str} (404 - Data not found)")
            print("   This is normal if your watch wasn't synced or no RHR data was recorded.")
            # Return empty data instead of None to continue processing
            return {"activities-heart": []}, token
        elif res.status_code == 500:
            print(f"WARNING: Server error for period {period} ending {end_str} (500 - API error)")
...
def fetch_resting_hr_chunk(start_str, end_str, token):
    url = f"https://api.fitbit.com/1/user/-/activities/heart/date/{start_str}/{end_str}.json"
    headers = {"Authorization": f"Bearer {token}"}
...
def parse_resting_hr(json_data):
    ...
    rows = []
    for entry in json_data["activities-heart"]:
        date = entry.get("dateTime")
        resting_hr = entry.get("value", {}).get("restingHeartRate")
        if resting_hr is not None:
            rows.append({"date": date, "resting_heart_rate": resting_hr})
...
def main():
    ...
    if last_date:
        ...
        # Use 90-day fallback directly - no need to fetch memberSince
        start_date = FALLBACK_START_DATE
        source = f"90-day fallback ({FALLBACK_START_DATE.strftime('%Y-%m-%d')})"
    
    print(f"Starting resting HR fetch from {start_date.strftime('%Y-%m-%d')} (source: {source})")
    ...
    token = get_access_token()
    json_data_p, token = fetch_resting_hr_period(end_str, period, token)
    if json_data_p is not None:
        df_p = parse_resting_hr(json_data_p)
        if not df_p.empty and "date" in df_p.columns:
            ...
            print("All available resting heart rate data fetched and saved.")
            print(f"Added {new_records} new resting heart rate records. Total records: {final_data_count}")
            return
    ...
    for start_str, end_str in daterange_chunks(start_date, end_date, CHUNK_DAYS):
        ...
        json_data, token = fetch_resting_hr_chunk(start_str, end_str, token)
        
        # Check if fetch was successful
        if json_data is None:
            failed_chunks += 1
            print(f"Failed to fetch data for {start_str} to {end_str}")
            print(f"ERROR: Failed to fetch data for {start_str} to {end_str}")
            continue
            
        df = parse_resting_hr(json_data)
    ...
    if successful_chunks == 0:
        print("SUCCESS: No new data available - all data is already up to date.")
    elif failed_chunks > 0:
        print(f"WARNING: Partial success: {successful_chunks}/{total_chunks} chunks fetched successfully.")
        print(f"Added {new_records} new resting heart rate records. Total records: {final_data_count}")
    else:
        print("SUCCESS: All available resting heart rate data fetched and saved.")
        print(f"Added {new_records} new resting heart rate records. Total records: {final_data_count}")
